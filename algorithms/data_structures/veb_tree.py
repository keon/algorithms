...
class VEBTree:
    ...
    def __init__(self, universe_size) -> None:
        ...
        if not isinstance(universe_size, int):
            msg = "universe_size must be an integer."
            raise TypeError(msg)
        if not universe_size > 0:
            msg = "universe_size must be greater than 0."
            raise ValueError(msg)
        if universe_size & universe_size - 1 != 0:
            msg = "universe_size must be a power of 2."
            raise ValueError(msg)
    ...
    def _validate_key(self, x) -> None:
        ...
        """
        if not (0 <= x < self.u):
            msg = f"Key {x} out of universe range [0, {self.u - 1}]"
            raise ValueError(msg)
