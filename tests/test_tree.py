...
from algorithms.tree import fenwick_tree
...
class TestFenwickTree(unittest.TestCase):
    def test_construct_tree_with_update_1(self):
        ...
        bit_tree = fenwick_tree.construct(freq, len(freq))
        self.assertEqual(12, fenwick_tree.get_sum(bit_tree, 5))

        freq[3] += 6
        fenwick_tree.update_bit(bit_tree, len(freq), 3, 6)
        self.assertEqual(18, fenwick_tree.get_sum(bit_tree, 5))

    def test_construct_tree_with_update_2(self):
        freq = [1, 2, 3, 4, 5]
        bit_tree = fenwick_tree.construct(freq, len(freq))
        self.assertEqual(10, fenwick_tree.get_sum(bit_tree, 3))

        freq[3] -= 5
        fenwick_tree.update_bit(bit_tree, len(freq), 3, -5)
        self.assertEqual(5, fenwick_tree.get_sum(bit_tree, 3))

    def test_construct_tree_with_update_3(self):
        freq = [2, 1, 4, 6, -1, 5, -32, 0, 1]
        bit_tree = fenwick_tree.construct(freq, len(freq))
        self.assertEqual(12, fenwick_tree.get_sum(bit_tree, 4))

        freq[2] += 11
        fenwick_tree.update_bit(bit_tree, len(freq), 2, 11)
        self.assertEqual(23, fenwick_tree.get_sum(bit_tree, 4))
